...
const validateAdminUser = async (user: IGetUser, requestRol: Rol): Promise<void> => {
    try {
        const thisUser = await User.findByPk(user.userId, { attributes: ['rol'] })
        if (requestRol == Rol.superAdmin) {
            if (thisUser.rol !== Rol.user && thisUser.rol == user.rol && thisUser.rol == requestRol) {
                return;
            } else {
                throw new UnauthorizedException()
            }
        } else {
            if ((thisUser.rol !== Rol.user && thisUser.rol == user.rol) && (thisUser.rol == requestRol || thisUser.rol == Rol.superAdmin)) {
                return;
            } else {
                throw new UnauthorizedException()
            }
        ...
        switch (error.constructor) {
            case UnauthorizedException:
                throw new UnauthorizedException('Tipo de usuario no autorizado para realizar esta operación');
            default:
                throw new InternalServerErrorException('Ocurrio un error en el servidor al verificar el tipo de usuario.')
        }
    }
}